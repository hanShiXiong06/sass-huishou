						...
						<u-row gutter="10" align="start">
							<u-col span="3">
								<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y"
									...
									style="height: 300px">
									<u-list height="auto">
										<u-list-item v-for="(item, index) in categoryList" :key="index">
											<u-cell :title="item.name" @click="changeActiveIndex(index)"
												style="margin: 10px 0px" :class="activeIndex == index ? 'active' : ''">
												<template #icon>
													<u-avatar shape="square" size="15"
								...
								<view class="right">
									<view v-if="categoryList.length != 0">
										<u-sticky bgColor="#fff">
											<u-tabs itemStyle="padding-left: 20px; padding-right: 20px; height: 34px;"
												:list="categoryList[activeIndex].children"></u-tabs>
										</u-sticky>

										<view class='modelList'> 
											<view v-for="(item, index) in categoryList[activeIndex].children"
												:key="index">
												<u-cell-group class="model_list" :title="item.name">
													<u-cell v-for="(model, indexs) in item.children" :key="indexs"
														:icon="'https://gl.hsxbk.top/' + model.icon" :title="model.name"
														isLink @click="linkModel(model.id)"></u-cell>
